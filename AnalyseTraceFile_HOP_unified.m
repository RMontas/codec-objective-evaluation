function [ ] = AnalyseTraceFile_HOP_unified( W, H )
% GEN CLEAN TRACE FILE : grep -v "="  TraceEnc.txt | grep -v "\-\-" | grep -v NAL | grep -v nuh | grep -v nal | grep -v BITS > TraceEnc_clean.txt

% YUV
RED(1)=76; RED(2)=84; RED(3)=255; % INTRA
GREEN(1)=149; GREEN(2)=43; GREEN(3)=21; % "SKIP"
BLUE(1)=29; BLUE(2)=255; BLUE(3)=107; % HOP-PROJ
ORANGE(1)=151; ORANGE(2)=42; ORANGE(3)=201; % HOP-BILIN
PINK(1)=105; PINK(2)=212; PINK(3)=234; % HOP-AFF
BLACK(1)=0; BLACK(2)=128; BLACK(3)=128; %
WHITE(1)=255; WHITE(2)=128; WHITE(3)=128; %
CYAN(1)=194; CYAN(2)=162; CYAN(3)=26; % MERGE
YELLOW(1)=225; YELLOW(2)=0; YELLOW(3)=148; % LOP
...
gt_aff_mode_counter_per_part_mode = zeros(8,1);
gt_proj_mode_counter_per_part_mode = zeros(8,1);
gt_bilin_mode_counter_per_part_mode = zeros(8,1);
...
pixel_counter_inter_gt_mode_aff = zeros(4,8,4);
pixel_counter_inter_gt_mode_proj = zeros(4,8,4);
pixel_counter_inter_gt_mode_bilin = zeros(4,8,4);
                ...
                if pred_mode_flag == 1 % INTRA
                    ...
                    if part_mode == 0
                        ...
                        if merge_flag % draw MRG shade
                            ...
                            if str2num(value{l+7})==0 && str2num(value{l+8})==1 % AFF
                                pixel_counter_inter_gt_mode_aff(log2(size)-2, part_mode + 1, 1) = ...
                                    pixel_counter_inter_gt_mode_aff(log2(size)-2, part_mode + 1, 1) + 1;
                                gt_aff_mode_counter_per_part_mode(part_mode + 1) = gt_aff_mode_counter_per_part_mode(part_mode + 1) +1;
                                for h = 1:size-1
                                    for w = 1:size-1
                                        Uframe(y+h,x+w)=PINK(2);
                                        Vframe(y+h,x+w)=PINK(3);
                                    end
                                end
                            elseif str2num(value{l+7})==1 && str2num(value{l+8})==0 % PROJ
                                pixel_counter_inter_gt_mode_proj(log2(size)-2, part_mode + 1, 1) = ...
                                    pixel_counter_inter_gt_mode_proj(log2(size)-2, part_mode + 1, 1) + 1;
                                gt_proj_mode_counter_per_part_mode(part_mode + 1) = gt_proj_mode_counter_per_part_mode(part_mode + 1) +1;
                                for h = 1:size-1
                                    for w = 1:size-1
                            ...
                            elseif str2num(value{l+7})==1 && str2num(value{l+8})==1 % BILIN
                                pixel_counter_inter_gt_mode_bilin(log2(size)-2, part_mode + 1, 1) = ...
                                    pixel_counter_inter_gt_mode_bilin(log2(size)-2, part_mode + 1, 1) + 1;
                                gt_bilin_mode_counter_per_part_mode(part_mode + 1) = gt_bilin_mode_counter_per_part_mode(part_mode + 1) +1;
                                for h = 1:size-1
                                    for w = 1:size-1
                                        Uframe(y+h,x+w)=ORANGE(2);
                                        Vframe(y+h,x+w)=ORANGE(3);
                                    end
                                end
                            else
                    ...
                    if part_mode == 1  % 2N x N horizontal
                        ...
                        if merge_flag % draw MRG shade
                            ...
                            if str2num(value{l+7})==0 && str2num(value{l+8})==1 % AFF
                                pixel_counter_inter_gt_mode_aff(log2(size)-2, part_mode + 1, 2) = ...
                                    pixel_counter_inter_gt_mode_aff(log2(size)-2, part_mode + 1, 2) + 1;
                                gt_aff_mode_counter_per_part_mode(part_mode + 1) = gt_aff_mode_counter_per_part_mode(part_mode + 1) +1;
                                for h = 1+size/2:size-1
                                    for w = 1:size-1
                                        Uframe(y+h,x+w)=PINK(2);
                            ...
                            elseif str2num(value{l+7})==1 && str2num(value{l+8})==0 % PROJ
                                pixel_counter_inter_gt_mode_proj(log2(size)-2, part_mode + 1, 2) = ...
                                    pixel_counter_inter_gt_mode_proj(log2(size)-2, part_mode + 1, 2) + 1;
                                gt_proj_mode_counter_per_part_mode(part_mode + 1) = gt_proj_mode_counter_per_part_mode(part_mode + 1) +1;
                                for h = 1+size/2:size-1
                                    for w = 1:size-1
                                        Uframe(y+h,x+w)=BLUE(2);
                            ...
                            elseif str2num(value{l+7})==1 && str2num(value{l+8})==1 % BILIN
                                pixel_counter_inter_gt_mode_bilin(log2(size)-2, part_mode + 1, 2) = ...
                                    pixel_counter_inter_gt_mode_bilin(log2(size)-2, part_mode + 1, 2) + 1;
                                gt_bilin_mode_counter_per_part_mode(part_mode + 1) = gt_bilin_mode_counter_per_part_mode(part_mode + 1) +1;
                                for h = 1+size/2:size-1
                                    for w = 1:size-1
                                        Uframe(y+h,x+w)=ORANGE(2);
                    ...
                    if part_mode == 3 % N x N
                        ...
                        if merge_flag % draw MRG shade
                            ...
                            if str2num(value{l+7})==0 && str2num(value{l+8})==1 % AFF
                                pixel_counter_inter_gt_mode_aff(log2(size)-2, part_mode + 1, 3) = ...
                                    pixel_counter_inter_gt_mode_aff(log2(size)-2, part_mode + 1, 3) + 1;
                                gt_aff_mode_counter_per_part_mode(part_mode + 1) = gt_aff_mode_counter_per_part_mode(part_mode + 1) +1;
                                for h = size/2+1:size-1
                                    for w = 1:size/2-1
                                        Uframe(y+h,x+w)=PINK(2);
                            ...
                            elseif str2num(value{l+7})==1 && str2num(value{l+8})==0 % PROJ
                                pixel_counter_inter_gt_mode_proj(log2(size)-2, part_mode + 1, 3) = ...
                                    pixel_counter_inter_gt_mode_proj(log2(size)-2, part_mode + 1, 3) + 1;
                                gt_proj_mode_counter_per_part_mode(part_mode + 1) = gt_proj_mode_counter_per_part_mode(part_mode + 1) +1;
                                for h = size/2+1:size-1
                                for w = 1:size/2-1
                                        Uframe(y+h,x+w)=BLUE(2);
                            ...
                            elseif str2num(value{l+7})==1 && str2num(value{l+8})==1 % BILIN
                                pixel_counter_inter_gt_mode_bilin(log2(size)-2, part_mode + 1, 3) = ...
                                    pixel_counter_inter_gt_mode_bilin(log2(size)-2, part_mode + 1, 3) + 1;
                                gt_bilin_mode_counter_per_part_mode(part_mode + 1) = gt_bilin_mode_counter_per_part_mode(part_mode + 1) +1;
                                for h = size/2+1:size-1
                                for w = 1:size/2-1
                                        Uframe(y+h,x+w)=ORANGE(2);
                        ...
                        if merge_flag % draw MRG shade
                            ...
                            if str2num(value{l+7})==0 && str2num(value{l+8})==1 % AFF
                                pixel_counter_inter_gt_mode_aff(log2(size)-2, part_mode + 1, 4) = ...
                                    pixel_counter_inter_gt_mode_aff(log2(size)-2, part_mode + 1, 4) + 1;
                                gt_aff_mode_counter_per_part_mode(part_mode + 1) = gt_aff_mode_counter_per_part_mode(part_mode + 1) +1;
                                for h = 1:size/2-1
                                    for w = size/2+1:size-1:size/2-1
                                        Uframe(y+h,x+w)=PINK(2);
                            ...
                            elseif str2num(value{l+7})==1 && str2num(value{l+8})==0 % PROJ
                                pixel_counter_inter_gt_mode_proj(log2(size)-2, part_mode + 1, 4) = ...
                                    pixel_counter_inter_gt_mode_proj(log2(size)-2, part_mode + 1, 4) + 1;
                                gt_proj_mode_counter_per_part_mode(part_mode + 1) = gt_proj_mode_counter_per_part_mode(part_mode + 1) +1;
                                for h = 1:size/2-1
                                    for w = size/2+1:size-1:size/2-1
                                        Uframe(y+h,x+w)=BLUE(2);
                            ...
                            elseif str2num(value{l+7})==1 && str2num(value{l+8})==1 % BILIN
                                pixel_counter_inter_gt_mode_bilin(log2(size)-2, part_mode + 1, 4) = ...
                                    pixel_counter_inter_gt_mode_bilin(log2(size)-2, part_mode + 1, 4) + 1;
                                gt_bilin_mode_counter_per_part_mode(part_mode + 1) = gt_bilin_mode_counter_per_part_mode(part_mode + 1) +1;
                                for h = 1:size/2-1
                                    for w = size/2+1:size-1:size/2-1
                                        Uframe(y+h,x+w)=ORANGE(2);
    ...
    for part_mode = 1 : 8
        for part_idx = 1 : 4
            ...
            pixel_counter_inter_mrg_mode(cu_size, part_mode, part_idx) = ...
                pixel_counter_inter_mrg_mode(cu_size, part_mode, part_idx) * ...
                pixel_counter_multiplier(cu_size, part_mode, part_idx);
            pixel_counter_inter_gt_mode_bilin(cu_size, part_mode, part_idx) = ...
                pixel_counter_inter_gt_mode_bilin(cu_size, part_mode, part_idx) * ...
                pixel_counter_multiplier(cu_size, part_mode, part_idx);
            pixel_counter_inter_gt_mode_aff(cu_size, part_mode, part_idx) = ...
                pixel_counter_inter_gt_mode_aff(cu_size, part_mode, part_idx) * ...
                pixel_counter_multiplier(cu_size, part_mode, part_idx);
            pixel_counter_inter_gt_mode_proj(cu_size, part_mode, part_idx) = ...
                pixel_counter_inter_gt_mode_proj(cu_size, part_mode, part_idx) * ...
                pixel_counter_multiplier(cu_size, part_mode, part_idx);
        end
    end
...
pixel_counter_inter_gt_total = 100 * sum(sum(sum(pixel_counter_inter_gt_mode_aff+pixel_counter_inter_gt_mode_proj+pixel_counter_inter_gt_mode_bilin))) / pixel_counter_total;
pixel_counter_inter_gt_aff_total = 100 * sum(sum(sum(pixel_counter_inter_gt_mode_aff))) / pixel_counter_total;
pixel_counter_inter_gt_proj_total = 100 * sum(sum(sum(pixel_counter_inter_gt_mode_proj))) / pixel_counter_total;
pixel_counter_inter_gt_bilin_total = 100 * sum(sum(sum(pixel_counter_inter_gt_mode_bilin))) / pixel_counter_total;
...
disp(pixel_counter_inter_gt_aff_total)
disp(pixel_counter_inter_gt_proj_total)
disp(pixel_counter_inter_gt_bilin_total)
...
disp(pixel_counter_inter_gt_mode_aff(1,:,:))
disp(pixel_counter_inter_gt_mode_proj(1,:,:))
disp(pixel_counter_inter_gt_mode_bilin(1,:,:))
disp('CTU size 16x16')
disp(pixel_counter_inter_me_mode(2,:,:))
disp(pixel_counter_inter_mrg_mode(2,:,:))
disp(pixel_counter_inter_gt_mode_aff(2,:,:))
disp(pixel_counter_inter_gt_mode_proj(2,:,:))
disp(pixel_counter_inter_gt_mode_bilin(2,:,:))
disp('CTU size 32x32')
disp(pixel_counter_inter_me_mode(3,:,:))
disp(pixel_counter_inter_mrg_mode(3,:,:))
disp(pixel_counter_inter_gt_mode_aff(3,:,:))
disp(pixel_counter_inter_gt_mode_proj(3,:,:))
disp(pixel_counter_inter_gt_mode_bilin(3,:,:))
disp('CTU size 64x64')
disp(pixel_counter_inter_me_mode(4,:,:))
disp(pixel_counter_inter_mrg_mode(4,:,:))
disp(pixel_counter_inter_gt_mode_aff(4,:,:))
disp(pixel_counter_inter_gt_mode_proj(4,:,:))
disp(pixel_counter_inter_gt_mode_bilin(4,:,:))
