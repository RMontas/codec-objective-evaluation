...
# Inputs: main_dir_name SEQ NSymbols AAC_Type
...
grep -v "="  $1/$i/$qp/TraceEnc.txt | grep -v "\-\-" | grep -v NAL | grep -v nuh | grep -v nal > $1/$i/$qp/TraceEnc_clean.txt
cp AnalyseTraceFile_v2.m $1/$i/$qp
cp yuv_export.m $1/$i/$qp
cp yuv_import.m $1/$i/$qp
cd $1/$i/$qp
/usr/local/MATLAB/R2015a/bin/matlab -nodesktop -nosplash -r "AnalyseTraceFile_v2(${W},${H},$3,$4);quit;" > outMATLAB.txt
cd ../../../
fi
done
done
...
grep -v "="  $1/$2/$qp/TraceEnc.txt | grep -v "\-\-" | grep -v NAL | grep -v nuh | grep -v nal > $1/$2/$qp/TraceEnc_clean.txt
cp AnalyseTraceFile_v2.m $1/$2/$qp
